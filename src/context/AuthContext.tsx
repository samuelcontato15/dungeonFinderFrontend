...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Busca dados completos do usuário no backend pelo ID (Mantendo a lógica do Samuel)
  ...
  // CORREÇÃO: Busca o ID real do usuário suportando busca por NICK ou EMAIL
  async function buscarIdPorNick(identificador: string): Promise<string> {
    try {
      ...
      console.error("DEBUG: Usuário não encontrado no backend para:", identificador);
      return "";
    } catch (err) {
  ...
  async function signIn(emailOrUser: string, senha: string) {
    ...
    // Tentamos buscar o ID usando o identificador original (pode ser nick ou email)
    const id = await buscarIdPorNick(emailOrUser);

    if (!id) {
      throw new Error("Não foi possível localizar o seu ID de usuário.");
    }
  ...
  // Mantendo a função de atualização para refletir mudanças do perfil no banco
  async function updateUser(updatedData: Partial<User>) {
    if (!user) return;

    const updatedUser = { ...user, ...updatedData };
    setUser(updatedUser);
    await AsyncStorage.setItem("@DungeonFinder:user", JSON.stringify(updatedUser));
  }
